...
# WebP Library Builder for Linux ARM64
# Builds libwebp.so for Linux ARM64 using Docker

$baseDir = $PSScriptRoot
$outputDir = Join-Path $baseDir "docker\build-webp\output\linux-arm64"
$dockerfile = Join-Path $baseDir "docker\build-webp\Dockerfile"
$imageName = "webp-builder-arm64"
$projectRuntimeDir = Join-Path $baseDir "src\LegioSoft.Imaging.WebP\runtimes\linux-arm64"

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "WebP Builder for Linux ARM64" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
docker build --target builder-arm64 -t $imageName -f $dockerfile $baseDir
...
docker cp "$containerName`:/output/linux-arm64" (Join-Path $baseDir "docker\build-webp\output")
